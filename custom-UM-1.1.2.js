...
function ___constructor_PullDown(obj){             //下拉内容过渡插件-构造函数
    ...
    this.unfold=function(){
        if(UM_PullDown.down.el.style.height!=='auto'){
            if(UM_PullDown.within && UM_PullDown.within.getStyle('opacity')!=0)UM_PullDown.within.css({height:'auto'});
    ...
    UM_PullDown.down.css({overflow:'hidden'});

    if(this.now===true){  // 设置初始状态为显示时的下拉框体样式
        UM_PullDown.down.css({opacity:1, height:UM_PullDown.height()});
    }else if(this.now===false){  // 设置初始状态为隐藏时的下拉框体样式
        UM_PullDown.down.css({opacity:0, height:0, paddingTop:0, paddingBottom:0, marginTop:0, marginBottom:0});
    }
    if(UM_PullDown.choosable===false)UM_PullDown.caption.choosable(false);  // 标题字体内容是否可被选中
    UM_PullDown.caption.css({cursor:'pointer'});  // 设置标题的cursor样式
    ...
    if(UM_PullDown.D_click===false){  // 当点击背景时, 折叠下拉框
        _(document).BD('click', function(){
            if(UM_PullDown.down.el.style.height==='auto')UM_PullDown.down.css({height:UM_PullDown.down.getStyle('height')});  // 为了使动画正常过渡, 需要设置height(当height的值为auto时, 改变height值不会显示动效)
    ...
    if(UM_PullDown.select===false){  // 当点击选项时, 折叠下拉框
        UM_PullDown.down.BD('click', function(event){
            _stopPropagation(event);
            if(UM_PullDown.within)UM_PullDown.within.css({height:'auto'});  // 当本插件为子嵌套插件时, 点击选项, 使父插件的down元素的height改成auto, 以使父插件的下拉框可以正确显示动效
            UM_PullDown.down.transition(UM_PullDown.speed+'s linear').transformOrigin('CENTER TOP').css({opacity:0, height:0, paddingTop:0, paddingBottom:0, marginTop:0, marginBottom:0});
            this.now=false;
        }.bind(this));
    }
