...
function _showingImg(asDomName, url, string){                       //按比例显示图片
    ...
    imgObj.onload=function(){
        ...
        if(parseInt(UM_showingImg_um.getStyle('width'))/parseInt(UM_showingImg_um.getStyle('height'))<UM_showingImg_w/UM_showingImg_h){
            if(_ifDom(asDomName))_('.'+UM_showingImg_name_class, 0).css({width:parseInt(UM_showingImg_um.getStyle('width'))*.98+'px', height:parseInt(UM_showingImg_um.getStyle('width'))*.98*UM_showingImg_h/UM_showingImg_w+'px', opacity:1});
            ...
            if(_ifDom(asDomName))_('.'+UM_showingImg_name_class, 0).css({width:parseInt(UM_showingImg_um.getStyle('height'))*.98*UM_showingImg_w/UM_showingImg_h+'px', height:parseInt(UM_showingImg_um.getStyle('height'))*.98+'px', opacity:1});
        }
    }
};
...
function _touchUpAndDown(UM_Obj){                    //移动端上下滑动推拽插件
    ...
    UM_touchUpAndDown.dragBox.BD('touchstart', function(){
        ...
        let _touchend=function(){
            _(document).unBD({touchmove:_touchmove, touchend:_touchend});
        };

        _(document).BD('touchmove', _touchmove);
