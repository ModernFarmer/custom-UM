...
function _showInputImg(json){                       //显示<input type="file" id/class="inputName">表单所选择的图片到指定的imgBox
    let UM_ShowInputImg={
    ...
    if(ifDom(UM_ShowInputImg.imgBoxName)){
        if(_(UM_ShowInputImg.imgBoxName, 0).el.children.length>0)_(UM_ShowInputImg.imgBoxName, 0).el.removeChild(_(UM_ShowInputImg.imgBoxName, 0).el.children[0]);
        let file=_(UM_ShowInputImg.inputName, 0).el.files;
        let imgObj=document.createElement('img');
        imgObj.className='UM_frame_img_custom_classname';               
        ...
        if(window.createObjectURL!=undefined){
            url=window.createObjectURL(file[0]);          //basic
        }else if(window.URL!=undefined){
            url=window.URL.createObjectURL(file[0]);      //firefox
        }else if(window.webkitURL!=undefined){
            url=window.webkitURL.createObjectURL(file[0]);   //chrome
        }
        imgObj.src=url;
        _(UM_ShowInputImg.imgBoxName, 0).el.appendChild(imgObj);
        _('.UM_frame_img_custom_classname', 0).transition('.2s ease').css({opacity:0});
        setTimeout(function(){
            if(_('.UM_frame_img_custom_classname', 0).el.offsetWidth!=0 && parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('width'))/parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('height'))<_('.UM_frame_img_custom_classname', 0).el.offsetWidth/_('.UM_frame_img_custom_classname', 0).el.offsetHeight){
                _('.UM_frame_img_custom_classname', 0).css({width:parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('width'))*.98+'px', height:'', opacity:1});
            }else if(_('.UM_frame_img_custom_classname', 0).el.offsetWidth!=0 && parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('width'))/parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('height'))>=_('.UM_frame_img_custom_classname', 0).el.offsetWidth/_('.UM_frame_img_custom_classname', 0).el.offsetHeight){
                _('.UM_frame_img_custom_classname', 0).css({width:'', height:parseInt(_(UM_ShowInputImg.imgBoxName, 0).getStyle('height'))*.98+'px', opacity:1});
            }
        },300);
        _('.UM_frame_img_custom_classname', 0).center();
    }
};
