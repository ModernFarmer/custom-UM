...
/*_MovingScroll({                          //滚动条插件    参数：json
    box:_('.box', 0),                   //容器盒子选择器  UM Selector       容器盒子不能加任何border、padding和margin相关的任何样式，如有需要可在容器盒子再套一个div来添加样式
    contentBox:_('.content', 0),        //滚动内容盒子选择器  UM Selector
    scrollBox:_('.scroll', 0),          //滚动条盒子选择器  UM Selector     滚动条盒子不能加任何border、padding和margin相关的任何样式
...
function _PullDown(obj){                           //下拉内容过渡插件
    ...
    let originalHeight=UM_PullDown.height();
    ...
    UM_PullDown.caption.BD('click', function(event){
        ...
        if(UM_PullDown.down.el.style.height!=='auto'){
            if(UM_PullDown.now===false){
                if(UM_PullDown.garret)UM_PullDown.garret.css({height:'auto'});
...
/*_PullDown({                                   //下拉内容过渡插件  参数：json
    caption:_('.caption', 0),             //标题选择器                                         UM Selector
    down:_('.down', 0),                   //下拉内容选择器                                     UM Selector
    garret:_('.garret', 0),               //嵌套的父元素选择器(即父嵌套插件的down参数)         UM Selector
...
/*_PullLeft({                               //左拉内容过渡插件  参数：json
    caption:_('.caption', 0),             //标题选择器                                         UM Selector
    left:_('.left', 0),                   //左拉内容选择器                                     UM Selector
    speed:1,                              //速度(在几秒内完成过渡)                             number      [可选, 默认0.5]
...
/*_Drag({               //拖拽插件，参数:json
  mousedownBox:_('.aaa', 0),                //被鼠标左键按下后发生拖拽的选择器      UM Selector
  dragBox:_('.box', 0),                     //被拖拽主体的选择器                    UM Selector       [可选, 默认为mousedownBox]
  parent:_('.bigbox', 0),                    //被拖拽元素的父级选择器               UM Selector       [可选, 默认为_('body',0)]
...
//_showingImg(asDomName, url, string)    //asDomName:  容器选择器名字   '.xxxx'
                          //url: 图片地址
